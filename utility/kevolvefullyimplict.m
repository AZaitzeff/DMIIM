function [x,y] = kevolvefullyimplict(dt,nt,x,y,S,M,closed,dir,rpar)
...
for t=1:nt
    ...
    Lp=0;
    for iter=1:MAXITER
        h=sqrt(((xl(3:n) - xl(1:n-2))).^2+((yl(3:n) - yl(1:n-2))).^2);
        L=sum(h);
        if abs(L-Lp)<tol
            break
        end
        Lp=L;
        k=1./sqrt((xl(2:n) - xl(1:n-1)).^2+(yl(2:n) - yl(1:n-1)).^2);
        z=2./h;
        B=zeros(n-2,3);
        ...
        %full(Ax)
        %full(Ay)
        xl(2:n-1) = mldivide(Ax,x(2:n-1)');
        yl(2:n-1) = mldivide(Ay,y(2:n-1)');
